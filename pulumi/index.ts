...
if (!sendgridApiKey)
    throw Error("SENDGRID_API_KEY not defined");
...
const localImageName = "contact-svc:" + imageVersion;

const imageName = repo + "/contact-svc:" + imageVersion;
		...
		containers: [
			...
			commands: [
			    "contact-svc",
                            "--enable-send",
                            "--enable-quiz",
                            "--subject=New contact from the Example Corp website",
                            "--from-email=hello@example.org",
                            "--to-email=hello@example.org"
			],
			ports: [
                            {
...
const serviceMon = new gcp.monitoring.GenericService(
	...
	displayName: "Contact Service (" + environment + ")",
	serviceId: "contact-svc-service-" + environment + "-mon",
	userLabels: {
	    "service": service.name,
	    "application": "contact-svc",
...
const latencySlo = new gcp.monitoring.Slo(
	...
	sloId: "contact-svc-service-" + environment + "-latency-slo",
	displayName: "Contact Service latency (" + environment + ")",
	goal: 0.95,
	rollingPeriodDays: 5,
	basicSli: {
	    latency: {
...
const availabilitySlo = new gcp.monitoring.Slo(
    ...
    {
	service: serviceMon.serviceId,
	sloId: "contact-svc-service-" + environment + "-availability-slo",
	displayName: "Contact Service availability (" + environment + ")",
	goal: 0.95,
	rollingPeriodDays: 5,
	windowsBasedSli: {
	    windowPeriod: "3600s",
